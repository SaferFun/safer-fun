  ...
  return (
    <div className="bg-theme-bg text-theme-text rounded-lg shadow-md p-6 transition-colors duration-300 border">
      <div className="flex flex-row justify-between items-center">
        ...
        <Link
          href={"https://github.com/SaferFun/safer-fun?tab=readme-ov-file#how-to-calculate-auction-score"}
          ...
          <tbody>
            {allocations.map((item, index) => (
              <tr key={index} className="border-t border-theme-text/10">
                ...
                <td className="py-2">{item.lockedSize.toFixed(2)} SOL</td>
                <td className="py-2">
                  {index === 0 ? '∞' : `${item.bidSize.toFixed(2)} SOL`}
                </td>
                <td className="py-2">
